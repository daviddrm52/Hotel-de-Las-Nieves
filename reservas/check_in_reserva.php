...
<?php 
    ...
    if(isset($_POST['checkin'])){
        ...
        //Creamos la query
        $queryCheckIn = "UPDATE 73_reservas SET estado_id = 2 WHERE id = '$id_a_actualizar'";

        //Creamos la query de certificacion de la reserva
        $queryResultadosCheckIn = "SELECT * FROM 73_reservas WHERE id = '$id_a_actualizar'";
 
        $resultadosCheckIn = mysqli_query($conn, $queryResultadosCheckIn);
        
        $confirmacionCheckIn = mysqli_fetch_array($resultadosCheckIn);
        
        $reservaConfirma = $confirmacionCheckIn['estado_id'];
        if($reservaConfirma=='1'){        
            //Lanzamos la query
            if(mysqli_query($conn, $queryCheckIn)){
                //La cosa ha ido bien
            ...
            $mensajeError="La reserva no esta en reservado, check-in no realizado.";
        }
    }
    ...
    if(isset($_GET['id'])){
        ...
        //Query de selección
        $sqlCheckIn = "SELECT * FROM 73_reservas WHERE id = $id";

        //Recuperamos los resultados
        $resultado = mysqli_query($conn, $sqlCheckIn);
...
<!DOCTYPE html>
<html lang="es">
<title> Check in anticipado - Hotel de Las Nieves </title>
...
<div class="container center white">
    ...
    <div class="container white center">
        <?php if($reserva):?>
            ...
            <h5>¿Desea realizar el check-in con antelación?</h5>
            <h5>Si lo realiza, disfrutará de un 25% de descuento en el SPA del hotel.</h5>
            <form action="check_in_reserva.php" method="POST">
                <input type="hidden" name="id_a_actualizar" value="<?php echo $reserva['id']?>">
                <button class="btn waves-effect waves-light brown darken-1" type="submit" name="checkin">Realizar Check-In anticipado
