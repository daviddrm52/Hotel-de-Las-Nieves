...
<?php
    ...
    $sql = "SELECT * FROM `73_reservas` WHERE `estado_id` = '2'";
...
<!DOCTYPE html>
<html lang="es">
<title> Mostrando reservas en check-in - Hotel de Las Nieves </title>
...
<div class="center fondoInicio">
  <h5> Mostrando todas las reservas que estan en "check-in" </h5>
</div>
...
<?php if(count($reservas) == 1) : ?>
    <p class="center"> <h6>Hay un total de <?php echo count($reservas)?> reserva en check-in.</h6> </p>
<?php elseif(count($reservas) >= 2) : ?>
    <p class="center"> <h6>Hay un total de <?php echo count($reservas)?> reservas en check-in.</h6> </p>
<?php else :?>
    <p class="center"> 
      <h6>No hay reservas en check-in.</h6> 
    </p>
<?php endif; ?>
</div>
