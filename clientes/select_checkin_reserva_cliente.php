...
<?php
    ...
    $sql = "SELECT * FROM 73_reservas WHERE cliente_id = '".$_SESSION["id"]."' AND estado_id = '2'";
...
<!DOCTYPE html>
<html lang="es">
<title> Reservas en check-in - Hotel de Las Nieves </title>
...
<div class="center fondoInicio">
  <h5> Mostrando reservas en check-in de <?php echo $usuarios['nombre']; ?> </h5>
</div>
...
<?php if(count($reservas) == 1) : ?>
    <p class="center"> <h6>Hay un total de <?php echo count($reservas)?> reserva en check-in a su nombre.</h6> </p>
<?php elseif(count($reservas) >= 2) : ?>
    <p class="center"> <h6>Hay un total de <?php echo count($reservas)?> reservas en check-in a su nombre.</h6> </p>
<?php else :?>
    <p class="center"> 
      <h6>No hay reservas en check-in a su nombre. </h6> 
    </p>
<?php endif; ?>
</div>
